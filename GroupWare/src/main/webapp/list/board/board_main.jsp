...
<link rel="stylesheet"
	href="<c:url value="/resources/css/board_main.css"/>">
</head>
<body>
	...
	<div class="container">
		<table class="table">
			...
			<tr>
				<td><%=board.getSeq()%></td>
				<td><a
					href="./BoardViewAction.do?num=<%=board.getSeq()%>&pageNum=<%=pageNum%>&number=<%=sessionNumber%>"><%=board.getTitle()%></a></td>
				<td><%=board.getName()%></td>
		...
		<div align="center">
			...
			<c:forEach var="i" begin="1" end="<%=total_page%>">
				...
				%>
				<a
					href="./board_main.do?pageNum=${i}&search_item=<%=search_item%>&text=<%=text%>" />
				<%
				} else {
				%>
		...
		<div align="right">
			<a href="/board_add.do?number=<%=sessionNumber%>"
				class="boardAdd btn btn-primary">글쓰기</a>
		</div>
				...
				<table>
					<tr>
						...
						<td class="search"><input class="search_content" type="text"
							name="text" /> <input type="submit" class="search_btn" value="검색" />
						</td>
					</tr>
				</table>
			</form>
